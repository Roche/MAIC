...
intervention to clinical trials of other treatments in a given disease area. For example if the only study of a new intervention is a single arm trial with no control group. This is commonly referred to as an unanchored MAIC.
...
outcomes, independent of the treatment received. For example, older patients may have increased risk of death compared to younger patients. Treatment effect
...
# Renames target population cols to be consistent with intervention_input
target_pop_standard <- target_pop %>%
  ...
  select(N, Treatment, AGE, AGE_SQUARED, SEX, SMOKE, ECOG0)
...
$x_{i,ipd}$ exactly matches the mean baseline characteristics for the comparator data source for which only aggregate data is available.[@signorovitch2010]

The weights are given by:
$$\hat{\omega}_i=\exp{(x_{i,ipd}.\beta)}\qquad (1)$$ That is, we must find a solution to: $$
\bar{x}_{agg}\sum_{i=1}^n \exp{(x_{i,ipd}.\beta)}  = \sum_{i=1}^n
x_{i,ipd}.\exp{(x_{i,ipd}.\beta)}\qquad (2)  $$ This estimator is equivalent to
solving the equation $$ 0 = \sum_{i=1}^n (x_{i,ipd} -  \bar{x}_{agg}
).\exp{(x_{i,ipd}.\beta)}\qquad (3)$$ without loss of generality, it can be assumed that
...
(x_{i,ipd})\exp{(x_{i,ipd}.\beta)}\qquad (4)$$ The right hand side of this estimator is
the first derivative of $$ Q(\beta) = \sum_{i=1}^n  \exp{(x_{i,ipd}.\beta)}\qquad (5) $$
As described by Signorovitch et al (supplemental appendix), $Q(\beta)$ is convex and therefore any finite solution to (2) is unique and corresponds to the global minimum of $Q(\beta)$.
...
As described by Phillippo et al., balancing on both mean and standard deviation for continuous variables (where possible) may be considered in some cases. This is included in the example below.[@phillippo2016a]
...
# (subtract the aggregate comparator data from the corresponding column of intervention IPD)
intervention_data <- intervention_input
for(i in match_cov){
  intervention_data[[paste0(i, "_centered")]] <- intervention_input[[i]] - target_pop_standard[,i]
}
...
to minimize $Q(\beta) = \sum_{i=1}^n \exp{(x_{i,ipd}.\beta)}$ and outputs a list
containing:
...
\hat{\omega}_i })^2}{ \sum_{i=1}^n \hat{\omega}_i^2} $$ A small ESS, relative
...
It is easier to examine the distribution of the weights by scaling them, so that the rescaled weights are relative to the original unit weights of each
...
```{r}
check_weights(est_weights$analysis_data, match_cov, target_pop_standard)
```
...
of Guyot et al. [@guyot2012] Recently, Liu et al. introduced an R package <tt>IPDfromKM</tt> that facilitates this method. [@Liu2021]
...
relationship between endpoints for a given patient since these are reconstructed data and not actual observed data.
...
To test the distribution of the bootstrapped ORs, a histogram can be plotted. If the plot does not appear normally distributed, this may suggest that the BCa
approach is more appropriate than the percentile approach.
